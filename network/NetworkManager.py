...
class NetworkTopo( Topo ):
    ...
    def generate_host_bw(self, host):
        global GLOBALS
        if host in GLOBALS.servers:
            # TODO: 26/11 => Minimizing BW s0 <-> hs ( *6 ==> *4 ==> *3) ==> Variable in GLOBALS.max_server_bw
            return self.get_server_host_max_bw()
        else:
            r = 0
            if host in GLOBALS.attackers:
                # TODO: 17/12 => BW attacker = BW Server
                # return GLOBALS.max_server_bw # TODO: 22/01/2024
    ...
    def generate_switch_bw(self, switch, dst, attacker_default_switch=''):
        ...
        if (GLOBALS.unified_switch_bandwidth is not None):
            ...
            if switch == 's0':
                # r = float((Decimal(random.randint(4, 10)) * Decimal('0.3')) + Decimal('0.1'))  # 1.3 --> 3.1
                # TODO: 17/12 => Customizing first attackers BW
                if False and attacker_default_switch == dst: # TODO: 22/01/2024
                    return self.get_server_host_max_bw()
                else:
                    # TODO: 17/12 => BW [1, 3.1]
    ...
    def build(self, **_opts):
        ...
        switch_grouper = SwitchGrouper(GLOBALS.controlled_switches_list)
        switch_groups = switch_grouper.get_switch_groups(GLOBALS.switch_groups_json_path)
        ...
        for switch in GLOBALS.controlled_switches_list:
            s0_bandwidthes[f"{GLOBALS.s0_switch}-{switch}"] = self.generate_switch_bw(GLOBALS.s0_switch, switch,
                                                                                      attacker_default_switch)
        ...
        for switch in GLOBALS.controlled_switches_list:
            ...
            if 'ports' not in GLOBALS.network_spec['switches'][switch]:
        ...
        for switch in GLOBALS.controlled_switches_list:
            if switch in GLOBALS.controlled_switch_to_router_relation:
                for router in GLOBALS.controlled_switch_to_router_relation[switch]['routers']:
                    GLOBALS.network_spec['switches'][switch]['ports'].append(shared.get_interface_name(switch, router))
        # Setup controlled switches connections based on groups
        for src_switch in GLOBALS.controlled_switches_list:
            connections = switch_groups['switch_neighbors'][src_switch]

            for dst_switch in connections:
                switch_key = f"{src_switch}-{dst_switch}"
                reversed_key = f"{dst_switch}-{src_switch}"

                # Generate bandwidth for the connection if it doesn't exist
                if switch_key not in switches_bandwidthes and reversed_key not in switches_bandwidthes:
                    bw = self.generate_switch_bw(src_switch, dst_switch)
                    switches_bandwidthes[switch_key] = bw
                    switches_bandwidthes[reversed_key] = bw

                # Get the bandwidth
                if switch_key in switches_bandwidthes:
                    switch_bw = switches_bandwidthes[switch_key]
                else:
                    switch_bw = switches_bandwidthes[reversed_key]

                # Setup interface names
                ...
                # Add ports to network_spec if they don't exist
                if 'ports' not in GLOBALS.network_spec['switches'][src_switch]:
                    GLOBALS.network_spec['switches'][src_switch]['ports'] = []
                if src_interface not in GLOBALS.network_spec['switches'][src_switch]['ports']:
                    GLOBALS.network_spec['switches'][src_switch]['ports'].append(src_interface)

                if 'ports' not in GLOBALS.network_spec['switches'][dst_switch]:
                    GLOBALS.network_spec['switches'][dst_switch]['ports'] = []
                if dst_interface not in GLOBALS.network_spec['switches'][dst_switch]['ports']:
                    GLOBALS.network_spec['switches'][dst_switch]['ports'].append(dst_interface)

                # Add connections to network_spec
                GLOBALS.network_spec['switches'][src_switch]['connections'][dst_switch] = {
                    'src_int': src_interface,
                    'dst_int': dst_interface,
                    'bw': f'{switch_bw}'
                }

                GLOBALS.network_spec['switches'][dst_switch]['connections'][src_switch] = {
                    'src_int': dst_interface,
                    'dst_int': src_interface,
                    'bw': f'{switch_bw}'
                }

        # Setup router switches
        ...
        for src_switch in GLOBALS.controlled_switches_list:
            connections_next = switch_groups['switch_neighbors_next'][src_switch]
            # Only create link if source switch index is less than destination switch index
            # to avoid creating duplicate links
            for dst_switch in connections_next:
                src_interface = shared.get_interface_name(src_switch, dst_switch)
                dst_interface = shared.get_interface_name(dst_switch, src_switch)
                ...
                self.addLink(src_switch, dst_switch, intfName1=src_interface, intfName2=dst_interface,
                             bw=float(
                                 GLOBALS.network_spec['switches'][src_switch]['connections'][dst_switch]['bw']),
                             max_queue_size=max_switch_queue_size)

        # Create links for router switches
        ...
        # Link server to s0
        info(
            f"*** Init link hs({GLOBALS.network_spec['hosts'][GLOBALS.default_server]['src_int']}) --> s0({GLOBALS.network_spec['hosts'][GLOBALS.default_server]['dst_int']}) with bw = {float(GLOBALS.network_spec['hosts'][GLOBALS.default_server]['bw'])}  ***\n")
        self.addLink(s0, GLOBALS.default_server,
                     intfName1=GLOBALS.network_spec['hosts'][GLOBALS.default_server]['dst_int'],
                     intfName2=GLOBALS.network_spec['hosts'][GLOBALS.default_server]['src_int'],
                     params2={'ip': "10.0.1.101/16"},
                     bw=float(GLOBALS.network_spec['hosts'][GLOBALS.default_server]['bw']),
                     max_queue_size=max_host_queue_size)

        # Link other hosts to their router switches
        for host in GLOBALS.client_hosts_list:
            router_switch = GLOBALS.network_spec['hosts'][host]['router_switch']
            info(
                f"*** Init link {host}({GLOBALS.network_spec['hosts'][host]['src_int']}) --> {router_switch}({GLOBALS.network_spec['hosts'][host]['dst_int']}) with bw = {float(GLOBALS.network_spec['hosts'][host]['bw'])}  ***\n")
            self.addLink(router_switch, host, intfName1=GLOBALS.network_spec['hosts'][host]['dst_int'],
